...
export default function Unverified() {
  ...
  async function fetchUnverifiedUsers() {
    try {
      let response = await fetch('http://localhost:3000/unverified');
  ...
  const submitSelectedIds = async () => {
    try {
      let response = await fetch('http://localhost:3000/verifyAll', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="p-6 bg-slate-950 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Unverified Students</h1>
        ...
        {showStudent.map((student, idx) => (
          ...
          >
            ...
            <FormControlLabel className="text-gray-300 "
             ...
             <button onClick={submitSelectedIds}  className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" >Verify</button>
            </div>
